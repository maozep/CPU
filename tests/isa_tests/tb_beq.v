...
//
// Test plan:
//   1. Load R1=5, R2=5 (equal), R3=10 (not equal) via ADDI
//   2. BEQ R1,R2,+1  -> should TAKE branch, skipping R4 write
//   3. BEQ R1,R3,+1  -> should NOT branch, R5 write executes
...
module tb_beq;
    ...
    initial begin
        ...
        $display("  R2 (ADDI R0+5)         Result: %0d  (expected 5)",  uut.regfile_inst.regs[2]);
        $display("  R3 (ADDI R0+10)        Result: %0d  (expected 10)", uut.regfile_inst.regs[3]);
        $display("  R4 (skipped by BEQ)    Result: %0d  (expected 0)",  uut.regfile_inst.regs[4]);
        ...
        if (uut.regfile_inst.regs[2] !== 8'd5)  begin $display("  FAIL: R2"); errors=errors+1; end
